...
def nextBlockClear(newBlock):
	...
	cur.execute("SELECT occupant FROM blocks WHERE block=%s" %(newBlock))
	occupant = cur.fetchall() #strip down result to the desired value
	occupant = occupant[0][0]
	if occupant == '0':
		cur.execute("UPDATE blocks SET occupant = %s WHERE occupant= %d" %('0',int(vehicle)))
		cur.execute("UPDATE blocks SET occupant = %s WHERE block=%d" %(vehicle,newBlock))
		advance = 1
	else:
		advance = 0
	conn.commit()
